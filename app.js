...
function difficultyClass(x){
  return x==='Легко'?'easy':(x==='Средняя'?'mid':'hard');
}
function logoClass(p){
  const map={ZetaChain:'zeta',Starknet:'stark',LayerZero:'layerzero','Kelp DAO':'kelp','Manta Network':'manta',Monad:'monad',Initia:'initia',Berachain:'bera'};
  return map[p.name]||'generic';
}
function projectCard(p){
  const fav=state.favorites.includes(p.id);
  const pot=scaledPotential(p);
  const isFree=p.minDeposit===0;
  return `<article class="op-card" data-open="${p.id}">
    <div class="op-top">
      <div class="op-logo ${logoClass(p)}">${p.logo}</div>
      <div class="op-main">
        <div class="op-name-row">
          <h3>${p.name}</h3>
          <span class="op-type">${p.type}</span>
        </div>
        <p>${p.desc}</p>
      </div>
      <button class="op-fav ${fav?'on':''}" data-fav="${p.id}" aria-label="Избранное">${fav?'★':'☆'}</button>
    </div>

    <div class="op-body">
      <div class="op-metrics">
        <div><small>Вложения</small><b>${isFree?'$0':'от '+money(p.minDeposit)}</b></div>
        <div><small>Потенциал</small><b class="green">${money(pot[0])} – ${money(pot[1])}</b></div>
        <div><small>Сложность</small><b class="${difficultyClass(p.difficulty)}">${p.difficulty}</b></div>
      </div>
      <div class="op-score"><b>${p.score}</b><small>/100</small></div>
    </div>

    <div class="op-tags">
      <span>${isFree?'Бесплатно':'с депозитом'}</span>
      <span>${p.time}</span>
      <span>Фонды: ${p.fundsCount}</span>
    </div>

    <div class="op-funds">${p.funds.slice(0,3).map(f=>`<span>${f}</span>`).join('')}</div>
  </article>`;
}
function render(){
  const list=filtered();
  ...
  app.innerHTML=`<main class="page">
    ...
    <section class="cr-source-card">
      <div class="cr-icon">C</div>
      <div class="cr-info">
        <h2>CryptoRank Scanner</h2>
        <p><span></span>Источник активен</p>
        <small>Анализируем airdrops, testnets, staking, launchpads и проекты с фондами</small>
      </div>
      <button class="check-btn" data-scan>Проверить</button>
    ...
    <section class="filters compact">
      <div class="fBlock">
        ...
        <div class="depositRow">${[50,100,500,1000].map(d=>`<button class="dep ${d===state.deposit?'active':''}" data-dep="${d}"><b>${d}</b><small>USDT</small></button>`).join('')}<button class="dep customBtn" data-custom><b>Своя</b><small>сумма</small></button></div>
      </div>
      <div class="cat-row">
        ${categories.map(c=>`<button class="cat ${state.category===c[0]?'active':''}" data-cat="${c[0]}">${c[1]}</button>`).join('')}
      </div>
    </section>
    ...
    <section class="list opportunities">${list.length?list.map(projectCard).join(''):'<div class="empty">Нет проектов по выбранному фильтру.</div>'}</section>
...
function bind(){
  ...
  if(sort) sort.onclick=()=>{state.sort=state.sort==='potential'?'score':'potential';save();render()};
  const pro=document.querySelectorAll('[data-pro]');
  pro.forEach(b=>b.onclick=()=>{state.pro=!state.pro;save();render()});
  const scan=document.querySelector('[data-scan]');
  if(scan) scan.onclick=()=>{scan.textContent='Сканирую...'; setTimeout(()=>{toast('CryptoRank Scanner обновлён'); render()},650)};
  document.querySelectorAll('[data-alerts]').forEach(b=>b.onclick=()=>toast('Уведомления будут на следующем этапе'));
}
render();
