...
function render(){const now=Date.now();let filtered=sorted(offers.filter(o=>exchanges.includes(o.ex)&&matchesType(o)&&o.active!==false&&(!o.endAt||new Date(o.endAt).getTime()>now)));if(favoritesOnly){filtered=filtered.filter(o=>fav.includes(o.ex+'-'+o.name))}const best=filtered[0];app.innerHTML=`<main class="page"><header class="top"><img class="logo" src="icon.svg"><div class="title"><h1>PromoRadar AI</h1><p>Акции топ-бирж в одном месте</p></div><button class="proTop" data-pro>${isPro?'PRO':'PRO'}</button></header>
${!isPro?'<section class="proMini"><b>Free режим</b><span>Демо-доступ. Полный доступ ко всем биржам — через Telegram Stars.</span><button data-pro>Открыть PRO</button></section>':''}
...
function proModal(){return `<div class="modal" id="proModal"><div class="sheet proSheet"><div class="sheetHead"><h2>PromoRadar PRO</h2><button data-pro-close class="close">×</button></div><p class="proLead">Полный доступ ко всем биржам, избранное, уведомления и быстрые переходы в разделы бирж.</p><div class="plans"><button class="plan" data-plan="day"><b>1 день</b><span>5 ⭐</span><small>проверить приложение</small></button><button class="plan bestPlan" data-plan="month"><b>1 месяц</b><span>30 ⭐</span><small>выгоднее дня</small></button><button class="plan" data-plan="half"><b>6 месяцев</b><span>120 ⭐</span><small>экономия</small></button><button class="plan" data-plan="year"><b>1 год</b><span>180 ⭐</span><small>самый выгодный</small></button></div><button class="tgHint" data-demo-pro>Включить PRO демо</button><div class="proFoot">В Telegram Mini App эти кнопки будут подключены к оплате Stars.</div></div></div>`}
...
// v39 Telegram Subscription + Favorites
