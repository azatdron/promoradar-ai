...
## v46 Real Source Foundation
- Скрыты демо-карточки до live-подтверждения официальным источником.
- Добавлен компактный блок «Источники» со статусом проверки бирж.
- В карточках остаются только активные подтверждённые предложения.
- Pull-to-refresh запускает live-проверку источников.
- API `/api/live` обновлён до real-source-foundation-v46.
